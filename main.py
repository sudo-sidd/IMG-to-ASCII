...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Output ASCII art height (auto if not set)",
    )
    parser.add_argument(
        "--charset",
        type=str,
        default=DEFAULT_CHARS,
        help="Characters to use for ASCII art",
    )
    parser.add_argument(
        "--colormode",
        choices=["grayscale", "ansi", "rgb"],
        default="ansi",
        help="Color mode",
    )
    ...
    parser.add_argument(
        "--keep-size",
        action="store_true",
        help="Use original image pixel dimensions (no resizing, 1 char per pixel)",
    )
...
def ascii_art_html(img, charset, colormode):
    ...
    html = (
        "<pre style='font: 10px/5px monospace; background: #000;'>"
        + "\n".join(html_lines)
        + "</pre>"
    )
    return html
